...
public class SimpleCart extends RandomizableClassifier implements
	...
	protected void makeTree(Instances data, int totalInstances,
		...
		for (int i = 0; i < data.numAttributes(); i++) {
			...
			if (att.isNumeric()) {
				// numeric attribute
				splits[i] = numericDistribution(props, dists, att,
						sortedIndices[i], weights[i], totalSubsetWeights,
						giniGains, data);
			} else {
				// nominal attribute
		...
		if (totalWeight < 2 * minNumObj || giniGains[attIndex] == 0
			...
			// If split of the node results in a node with less than minimal
			// number of isntances,
			...
			for (int i = 0; i < 2; i++) {
				...
				m_Successors[i].makeTree(data, m_totalTrainInstances,
						subsetIndices[i], subsetWeights[i], dists[attIndex][i],
						totalSubsetWeights[attIndex][i], minNumObj,
						useHeuristic);
			}
		}
	}
		...
		for (i = 0; i < sortedIndices.length; i++) {
			...
			if (inst.value(att) > currSplit) {
				...
				for (int k = 0; k < 2; k++) {
					...
					System.arraycopy(currDist[k], 0, tempDist[k], 0,
							tempDist[k].length);
				}
	...
	protected String nominalDistribution(double[][] props, double[][][] dists,
		...
		if (data.numClasses() == 2) {
			...
			for (int i = 0; i < sortedIndices.length; i++) {
				...
				for (int j = 0; j < nonEmpty; j++) {
					if (att.value((int) inst.value(att)).compareTo(
							nonEmptyValues[j]) == 0) {
			...
			for (int j = 0; j < nonEmpty - 1; j++) {
				...
				for (int i = 0; i < sortedIndices.length; i++) {
					...
					if (tempStr.indexOf("(" + att.value((int) inst.value(att))
							+ ")") != -1) {
						currDist[0][(int) inst.classValue()] += weights[i];
					} else
						currDist[1][(int) inst.classValue()] += weights[i];
	...
	protected void splitData(int[][][] subsetIndices,
			double[][][] subsetWeights, Attribute att, double splitPoint,
			String splitStr, int[][] sortedIndices, double[][] weights,
				...
				if (inst.isMissing(att)) {
					...
					for (int k = 0; k < 2; k++) {
						if (m_Props[k] > 0) {
							...
							subsetWeights[k][i][num[k]] = m_Props[k]
									* weights[i][j];
							num[k]++;
						}
					}
					...
					if (att.isNumeric()) {
						...
						if (splitStr.indexOf("("
								+ att.value((int) inst.value(att.index()))
								+ ")") != -1) {
							subset = 0;
						} else
							subset = 1;
		...
		if (!m_isLeaf) {
			...
			// split attribute is nonimal
				...
				if (m_SplitString.indexOf("("
						+ m_Attribute.value((int) instance.value(m_Attribute))
						+ ")") != -1)
